...
{	
    ...
    public Moon(Context context,float r)
    {    	
    	//初始化顶点坐标与着色数据
    	initVertexData(r);
    	//初始化shader        
    ...
    {
    	//加载顶点着色器的脚本内容       
        mVertexShader=ShaderUtil.loadFromAssetsFile("vertex_moon.sh", context.getResources());
        ShaderUtil.checkGlError("==ss==");   
        //加载片元着色器的脚本内容
        mFragmentShader=ShaderUtil.loadFromAssetsFile("frag_moon.sh", context.getResources());  
        //基于顶点着色器与片元着色器创建程序
        ShaderUtil.checkGlError("==ss==");      
