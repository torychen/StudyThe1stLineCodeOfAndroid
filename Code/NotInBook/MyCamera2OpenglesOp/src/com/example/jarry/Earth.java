...
public class Earth  {	
    ...
    int vCount=0; 
    public Earth(Context context,float r){    	
    	//初始化顶点坐标与着色数据
    	initVertexData(r);
    	//初始化着色器        
    	initShader(context);
    ...
    public void initShader(Context context){ //初始化着色器
    	//加载顶点着色器的脚本内容       
        mVertexShader=ShaderUtil.loadFromAssetsFile("vertex_earth.sh", context.getResources());
        ShaderUtil.checkGlError("==ss==");   
        //加载片元着色器的脚本内容
        mFragmentShader=ShaderUtil.loadFromAssetsFile("frag_earth.sh", context.getResources());  
        //基于顶点着色器与片元着色器创建程序
        ShaderUtil.checkGlError("==ss==");      
